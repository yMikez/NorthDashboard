...
- get_affiliate_sequence: K janelas em SEQUÊNCIA (Janela 1..K) — ranking de cada janela, transições explicadas, evolução por afiliado com tags/comentários, quem está parando de rodar (slowing), reativação e saúde da base. Use pra "como foram as últimas 3 semanas?", "quem parou?", "evolução do X". Todas as tools de janela aceitam \`anchor\` (janela personalizada até um dia) e \`include_today\`.
- get_funnel: take rate e receita por etapa, por família (período). get_funnel_sequence: o funil em K janelas em sequência com a variação explicada (volume de FEs × AOV de sessão, estágio que mais mexeu) — use pra "o funil piorou?", "qual upsell caiu?", "foi volume ou conversão?". get_products: TODOS os SKUs com métricas. get_families: visão por família.
