const { useState: useStateApp, useEffect: useEffectApp } = React;

// --- route ↔ metadata
const ROUTES = {
  'overview':       { title: 'Visão',       em: 'geral',       crumbs: ['Análise', 'Visão geral'] },
  'funnel':         { title: 'Funil',       em: 'completo',    crumbs: ['Análise', 'Funil'] },
  'custos':         { title: 'Custos',      em: 'e lucro',     crumbs: ['Análise', 'Custos'] },
  'leaderboard':    { title: 'Ranking',     em: 'de afiliados', crumbs: ['Afiliados', 'Ranking'] },
  'all-affiliates': { title: 'Diretório',   em: 'de afiliados', crumbs: ['Afiliados', 'Todos'] },
  'recovery':       { title: 'Recuperação', em: 'de vendas',   crumbs: ['Afiliados', 'Recuperação'] },
  'products':       { title: 'Produtos',     em: '',            crumbs: ['Catálogo', 'Produtos'] },
  'transactions':   { title: 'Transações',  em: 'do ledger',   crumbs: ['Catálogo', 'Transações'] },
  'platforms':      { title: 'Plataformas', em: 'conectadas',  crumbs: ['Sistema', 'Plataformas'] },
  'health':         { title: 'Saúde',       em: 'do dado',     crumbs: ['Sistema', 'Saúde do dado'] },
  'costs':          { title: 'Fulfillment', em: 'e custo de envio', crumbs: ['Sistema', 'Fulfillment'] },
  'chat':           { title: 'Análise',    em: 'com IA',         crumbs: ['Admin', 'Chat'] },
  'insights':       { title: 'Insights',    em: 'do negócio',  crumbs: ['Análise', 'Insights'] },
  'users':          { title: 'Usuários',    em: 'do dashboard', crumbs: ['Admin', 'Usuários'] },
  'networks':       { title: 'Networks',    em: 'parceiras',   crumbs: ['Admin', 'Networks'] },
  'copy-optimizer': { title: 'Copy',         em: 'Optimizer',   crumbs: ['Admin', 'Copy Optimizer'] },
};

// Path-based routing via History API. Pathname is the route name (e.g.
// "/leaderboard"); querystring carries filter state. We also accept the
// legacy hash format ("#leaderboard?...") so old bookmarks keep working —
// when detected, we silently rewrite the URL to the clean form on first
// load.
function readRoute() {
  // Legacy hash bookmark: "/index.html#leaderboard?range=30d" or "/#funnel"
  if (location.hash) {
    const h = location.hash.replace(/^#/, '');
    const [route, q] = h.split('?');
    if (ROUTES[route]) {
      const params = new URLSearchParams(q || '');
      // Rewrite to the clean URL — keep the same browser entry, no reload.
      const newUrl = '/' + route + (q ? '?' + q : '');
      history.replaceState(null, '', newUrl);
      return { route, params };
    }
  }
  let path = location.pathname.replace(/^\//, '').replace(/\/$/, '');
  // /index.html (legacy direct hit) redirects to clean root.
  if (path === 'index.html') {
    history.replaceState(null, '', '/' + location.search);
    path = '';
  }
  const route = ROUTES[path] ? path : 'overview';
  const params = new URLSearchParams(location.search);
  return { route, params };
}
function writeRoute(route, params) {
  const qs = params.toString();
  const url = '/' + route + (qs ? '?' + qs : '');
  history.pushState(null, '', url);
}

// Helper global pra drill-down (KpiCard click, hbar click, etc).
// Navega pra outra rota com parâmetros adicionais opcionais.
// Preserva os params atuais e mescla os novos. Dispara popstate pra
// que o App reaja sem reload.
window.NSNavigate = function(route, extraParams) {
  const params = new URLSearchParams(location.search);
  if (extraParams) {
    for (const k of Object.keys(extraParams)) {
      const v = extraParams[k];
      if (v == null || v === '') params.delete(k);
      else params.set(k, v);
    }
  }
  const qs = params.toString();
  const url = '/' + route + (qs ? '?' + qs : '');
  window.history.pushState(null, '', url);
  window.dispatchEvent(new PopStateEvent('popstate'));
};

function decodeSet(s) { return new Set(s ? s.split(',').filter(Boolean) : []); }
function encodeSet(set) { return Array.from(set).join(','); }

function App({ user }) {
  // NETWORK_PARTNER tem shell próprio (PartnerShell) — não compartilha a
  // sidebar/topbar/filtros do dashboard admin/member. Bypass total.
  if (user?.role === 'NETWORK_PARTNER') {
    async function logout() {
      try {
        await fetch('/api/auth/logout', { method: 'POST' });
      } catch (e) { /* swallow */ }
      window.location.href = '/login';
    }
    return <window.PartnerShell user={user} onLogout={logout}/>;
  }
  // Resolve a default route a partir do allowedTabs, pra MEMBER cair em
  // alguma tab que ele tenha acesso (em vez de cair em 'overview' e ver
  // página em branco).
  const isAdminApp = user?.role === 'ADMIN';
  const allowedRoutesApp = isAdminApp ? Object.keys(ROUTES) : (user?.allowedTabs || []);
  const initialState = (() => {
    const r = readRoute();
    if (isAdminApp || allowedRoutesApp.includes(r.route)) return r;
    const fallback = allowedRoutesApp[0] || 'overview';
    return { route: fallback, params: r.params };
  })();
  const [hashState, setHashState] = useStateApp(initialState);
  const [filterOptions, setFilterOptions] = useStateApp(null);

  // popstate fires on browser back/forward. We re-read the URL and update
  // route + filter state. Hash changes still get caught (legacy bookmarks)
  // because readRoute() handles both forms.
  useEffectApp(() => {
    const fn = () => setHashState(readRoute());
    window.addEventListener('popstate', fn);
    return () => window.removeEventListener('popstate', fn);
  }, []);

  // Load real filter options once on mount. We don't refresh on every
  // navigation — the universe of platforms/funnels/countries changes only
  // when new orders arrive, which is rare relative to user interaction.
  useEffectApp(() => {
    window.NSApi.fetchFilterOptions()
      .then((data) => setFilterOptions(data))
      .catch((err) => console.error('fetchFilterOptions failed', err));
  }, []);

  const params = hashState.params;
  const initialPreset = params.get('range') || '30d';
  // For custom ranges, the preset is 'custom' and the actual dates come from
  // 'from'/'to' params (YYYY-MM-DD). If they're missing or malformed, fall
  // back to the 30d default so the page still loads.
  const initialRange = (() => {
    if (initialPreset === 'custom') {
      const fromRaw = params.get('from');
      const toRaw = params.get('to');
      const start = fromRaw ? new Date(fromRaw + 'T00:00:00.000Z') : null;
      const end = toRaw ? new Date(toRaw + 'T23:59:59.999Z') : null;
      if (start && end && !Number.isNaN(start.getTime()) && !Number.isNaN(end.getTime()) && start <= end) {
        return { start, end, preset: 'custom' };
      }
      return rangeForPreset('30d');
    }
    return rangeForPreset(initialPreset);
  })();

  const [filters, setFilters] = useStateApp(() => ({
    preset: initialRange.preset || initialPreset,
    dateRange: initialRange,
    compare: params.get('cmp') === '1',
    platforms: decodeSet(params.get('plat')),
    funnels: decodeSet(params.get('of')),
    families: decodeSet(params.get('fam')),
    countries: decodeSet(params.get('co')),
    // Etapa do funil — front/upsell/downsell/recuperacao. Vai pro
    // backend como query param `stages` (CSV) que mapeia pra
    // Order.productType (FRONTEND/UPSELL/DOWNSELL/SMS_RECOVERY).
    stages: decodeSet(params.get('st')),
    // Retained for backward-compat with pages still reading filters.trafficSources / filters.currency.
    // Both removed from FilterBar UI + URL persistence in Fase 2A cleanup.
    trafficSources: new Set(),
    currency: 'USD',
  }));

  // persist filters to URL via replaceState (no new history entry — typing
  // in a filter shouldn't litter back-button history).
  useEffectApp(() => {
    const p = new URLSearchParams();
    if (filters.preset !== '30d') p.set('range', filters.preset);
    if (filters.preset === 'custom') {
      p.set('from', isoDateOnly(filters.dateRange.start));
      p.set('to', isoDateOnly(filters.dateRange.end));
    }
    if (filters.compare) p.set('cmp', '1');
    if (filters.platforms.size) p.set('plat', encodeSet(filters.platforms));
    if (filters.funnels.size) p.set('of', encodeSet(filters.funnels));
    if (filters.families.size) p.set('fam', encodeSet(filters.families));
    if (filters.countries.size) p.set('co', encodeSet(filters.countries));
    if (filters.stages?.size) p.set('st', encodeSet(filters.stages));
    const qs = p.toString();
    const desiredUrl = '/' + hashState.route + (qs ? '?' + qs : '');
    const currentUrl = location.pathname + location.search;
    if (currentUrl !== desiredUrl) {
      history.replaceState(null, '', desiredUrl);
    }
  }, [filters, hashState.route]);

  const [drawerAff, setDrawerAff] = useStateApp(null);

  function onNav(route) {
    // /chat foi migrado pra rota Next.js native (TS + Tailwind + shadcn).
    // SPA não renderiza mais o ChatPage inline — full-page nav garante que
    // o usuário vê a UI nova com layout integrado (dashboard nav + chat).
    if (route === 'chat') {
      window.location.href = '/chat';
      return;
    }
    setDrawerAff(null);
    // Preserve current querystring (filter state) when switching routes.
    writeRoute(route, new URLSearchParams(location.search));
    setHashState({ route, params: new URLSearchParams(location.search) });
  }

  const r = ROUTES[hashState.route];

  return (
    <div className="app">
      <FXLayers/>
      <Sidebar active={hashState.route} onNav={onNav} user={user}/>
      <div className="main">
        <Topbar
          title={r.title} titleEm={r.em} crumbs={r.crumbs}
          currency={filters.currency}
          onToggleCurrency={(c) => setFilters(f => ({ ...f, currency: c }))}
        />
        <FilterBar filters={filters} setFilters={setFilters} options={filterOptions} route={hashState.route}/>
        <div className="page">
          {hashState.route === 'overview'       && <OverviewPage filters={filters} setFilters={setFilters}/>}
          {hashState.route === 'funnel'         && <FunnelPage filters={filters}/>}
          {hashState.route === 'leaderboard'    && <LeaderboardPage filters={filters} onOpenAffiliate={setDrawerAff}/>}
          {hashState.route === 'all-affiliates' && <AllAffiliatesPage filters={filters} onOpenAffiliate={setDrawerAff}/>}
          {hashState.route === 'recovery'       && <RecoveryPage filters={filters}/>}
          {hashState.route === 'products'       && <ProductsPage filters={filters}/>}
          {hashState.route === 'transactions'   && <TransactionsPage filters={filters}/>}
          {hashState.route === 'platforms'      && <IntegrationsPage filters={filters}/>}
          {hashState.route === 'health'         && <HealthPage/>}
          {hashState.route === 'costs'          && <CostsPage filters={filters}/>}
          {hashState.route === 'custos'         && <CustosPage filters={filters}/>}
          {/* 'chat' agora vive em /chat (Next.js native). Nav redireciona via window.location. */}
          {hashState.route === 'insights'       && <InsightsPage/>}
          {hashState.route === 'users'          && <UsersPage currentUser={user}/>}
          {hashState.route === 'networks'       && <NetworksPage/>}
          {hashState.route === 'copy-optimizer' && <CopyOptimizerPage/>}
        </div>
      </div>

      {drawerAff && <AffiliateDrawer affiliateId={drawerAff} filters={filters} onClose={() => setDrawerAff(null)}/>}
      <ChatWidget user={user}/>
      <FeesReviewPrompt user={user} setHashState={setHashState}/>
    </div>
  );
}

// Pop-up 7d: a cada 7 dias após a última atualização das taxas+allowance
// da Digistore (ou de qualquer plataforma com fees cadastradas), prompta
// o admin a reconfirmar os valores. Skip dura 24h via localStorage pra
// não martirizar quem dispensa.
function FeesReviewPrompt({ user, setHashState }) {
  const [stale, setStale] = useStateApp([]);
  const [dismissed, setDismissed] = useStateApp(false);

  useEffectApp(() => {
    if (!user || user.role !== 'ADMIN') return;
    // Snooze 24h após dismiss/save
    try {
      const until = parseInt(localStorage.getItem('ns-fees-prompt-dismissed-until') || '0', 10);
      if (Date.now() < until) { setDismissed(true); return; }
    } catch {}

    let cancelled = false;
    // Pega plataformas (sem filtro — só pra ver feesUpdatedAt).
    const now = new Date();
    const start = new Date(now.getTime() - 30 * 24 * 3600 * 1000);
    window.NSApi.fetchPlatforms({
      dateRange: { start, end: now },
      platforms: new Set(), countries: new Set(), funnels: new Set(), families: new Set(), stages: new Set(),
    }).then((data) => {
      if (cancelled) return;
      const SEVEN_DAYS = 7 * 24 * 3600 * 1000;
      const staleOnes = (data.platforms || []).filter((p) => {
        if (p.feeRatePct == null && p.allowancePct == null) return false; // não cadastrado
        if (!p.feesUpdatedAt) return true;
        return Date.now() - new Date(p.feesUpdatedAt).getTime() > SEVEN_DAYS;
      });
      setStale(staleOnes);
    }).catch(() => { /* silencioso */ });
    return () => { cancelled = true; };
  }, [user?.id]);

  function snooze() {
    try { localStorage.setItem('ns-fees-prompt-dismissed-until', String(Date.now() + 24 * 3600 * 1000)); } catch {}
    setDismissed(true);
  }
  function goReview() {
    setHashState({ route: 'platforms', params: new URLSearchParams() });
    writeRoute('platforms', new URLSearchParams());
    snooze();
  }

  if (dismissed || stale.length === 0) return null;

  return (
    <div style={{
      position: 'fixed', inset: 0, zIndex: 90, background: 'rgba(3,6,23,0.6)',
      backdropFilter: 'blur(6px)', display: 'grid', placeItems: 'center',
    }}>
      <div className="panel" style={{ width: 420, padding: 22 }}>
        <div className="eyebrow" style={{ fontSize: 10, color: 'var(--warning)', marginBottom: 4 }}>
          REVISÃO PERIÓDICA · 7 DIAS
        </div>
        <h3 style={{ margin: '0 0 4px', fontSize: 18 }}>
          Atualizar taxas <em style={{ color: 'var(--glow-cyan)' }}>e allowance</em>
        </h3>
        <p style={{ fontSize: 12, color: 'var(--fg3)', lineHeight: 1.55, marginBottom: 16 }}>
          As taxas e allowance percentuais não são atualizadas há mais de 7 dias em{' '}
          {stale.length === 1
            ? <strong>{stale[0].displayName}</strong>
            : <><strong>{stale.length} plataformas</strong></>}.
          Reconfirme pra manter os valores absolutos do card precisos.
        </p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 16 }}>
          {stale.map((p) => (
            <div key={p.slug} style={{
              fontSize: 11, fontFamily: 'var(--f-mono)', color: 'var(--fg4)',
              display: 'flex', justifyContent: 'space-between',
            }}>
              <span>{p.displayName}</span>
              <span>{p.feesUpdatedAt ? fmtSyncAgo(p.feesUpdatedAt) : 'nunca'}</span>
            </div>
          ))}
        </div>
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          <button className="btn btn-ghost" onClick={snooze}>Depois (24h)</button>
          <button className="btn btn-primary" onClick={goReview}>Atualizar agora</button>
        </div>
      </div>
    </div>
  );
}

// Bootstrap: fetch session info before rendering. 401 → /login. Em qualquer
// caso de sucesso, passa o user pro App pra filtrar sidebar + topbar.
async function bootstrap() {
  let user = null;
  try {
    const res = await fetch('/api/me', { credentials: 'same-origin' });
    if (res.status === 401) {
      const next = encodeURIComponent(location.pathname + location.search);
      window.location.href = '/login' + (next !== '%2F' ? '?next=' + next : '');
      return;
    }
    if (!res.ok) {
      console.error('GET /api/me returned', res.status);
    } else {
      const data = await res.json();
      user = data.user;
    }
  } catch (err) {
    console.error('GET /api/me failed', err);
  }
  if (!user) {
    window.location.href = '/login';
    return;
  }
  const root = ReactDOM.createRoot(document.getElementById('app'));
  root.render(<App user={user}/>);
}
bootstrap();
